 ...
 */
export default function Sparkline({ prices = [], width = 600, height = 80, strokeColor = 'rgb(99 102 241)', fillColor = 'rgba(99,102,241,0.15)' }) {
    if (!prices || prices.length < 2) {
        return (
            <div style={{ width, height }} className="flex items-center justify-center text-sm text-gray-500">
                No data
            </div>
        );
    }
    ...
    return (
        <svg width={width} height={height} viewBox={`0 0 ${width} ${height}`} role="img" aria-label="sparkline">
            ...
            <polyline fill="none" stroke={dynamicStroke} strokeWidth="2" strokeLinejoin="round" strokeLinecap="round" points={points} />
        </svg>
    );
}
