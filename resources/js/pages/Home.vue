    ...
    <div class="min-h-screen bg-slate-50 text-gray-700">
        ...
        <main class="mx-auto max-w-5xl px-4 py-6">
            <template v-if="loading">
                <div class="rounded border border-slate-200 bg-white p-4 text-gray-600">Loading assets…</div>
            </template>
                ...
                <div class="mt-5 grid grid-cols-1 gap-5 sm:grid-cols-2 lg:grid-cols-3">
                    ...
                    >
                        <a :href="`/assets/${a.id}`">
                            ...
                            <p class="ml-16 text-xl font-semibold text-gray-800 sm:pb-7">{{ formatted(a.current_price) }}</p>
                        </a>
                        <div class=""></div>
                        <div class="absolute inset-x-0 bottom-0 flex items-center justify-between bg-gray-50 px-4 py-4 sm:px-6">
                            <p
                                :class="[
                            ...
                            >
                                <ArrowUpIcon
                                    ...
                                    class="mr-0.5 size-4 shrink-0 self-center text-green-500"
                                    aria-hidden="true"
                                />
                                <ArrowDownIcon v-else class="mr-0.5 size-4 shrink-0 self-center text-red-500" aria-hidden="true" />
                            ...
                            >
                                <HeartIcon
                                    :class="[a.is_favorite ? 'text-indigo-400' : 'text-gray-300', 'mr-0.5 size-4 shrink-0 self-center']"
                                    class=""
                                    aria-hidden="true"
                                />
                            </button>
            ...
            <template v-else>
                <div class="rounded border border-slate-200 bg-white p-4 text-gray-600">No assets to display.</div>
            </template>
        </main>
    </div>
</template>
